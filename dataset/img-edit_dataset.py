...
class ImgEditClassificationDataset(Dataset):

    def __init__(self, transform, split, debug=False):
        data_dir = '/home/mila/b/benno.krojer/scratch/img-edit'
        super().__init__()
    ...
    def load_data(self, data_dir, split):
        ...
        for i, row in tqdm(enumerate(json_file), total=len(json_file)):
            ...
            image0_file = os.path.join(data_dir, "images", img0)
            image1_file = os.path.join(data_dir, "images", img1)
