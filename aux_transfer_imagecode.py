...
def evaluate(model, data_loader, tokenizer, device):    
    ...
    for i,(image, text, target) in enumerate(tqdm(data_loader)):
        ...
        matching_score = matching_score.reshape(-1, 2)
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, args):  
    ...
    total_loss = 0
    for i,(image, text, target) in enumerate(tqdm(data_loader, desc='batch')):
        image = image.to(device,non_blocking=True)   
        image = image.flatten(end_dim=1)
        target = target.to(device,non_blocking=True)
        if args.binary_cross_entropy:
            target = F.one_hot(target, num_classes=2).flatten()
        text_ = []
        for t in text:
            text_ += [t]*2
        text_input = tokenizer(text_, padding='longest', return_tensors="pt").to(device)  #TODO: max_length
...
def main(args, config):
    ...
    data_dir = '/Scratch/qing_meng/torch/ALBEF/' + args.dataset_dir
    if args.dataset_dir == "spotdiff":
    ...
    elif args.dataset_dir == "clevr_change":
    ...
    for epoch in tqdm(range(max_epoch), desc='epoch'):
        ...
        if accuracy > best:
            ...
            torch.save(save_obj, os.path.join(args.output_dir, f'checkpoint_{args.dataset_dir}_cls.pth')) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', default='spotdiff')
    ...
    args.output_dir = os.path.join(args.output_dir, args.dataset_dir)
